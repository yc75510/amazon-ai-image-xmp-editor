  ...
  async function addFiles(files: File[]) {
    const incoming = await Promise.all(files.map(async file => {
      ...
      try {
        ...
        if (location?.xml && tags.length && tags.every(tag => location.xml.includes(tag))) return { id, file, bytes, format, location, status: "already" as const, detail: "Tag already found" };
  ...
  function processAll() {
    ...
    if (mode === "detect") {
      setItems(current => current.map(item => {
        ...
        const found = Boolean(item.location?.xml && tags.every(tag => item.location?.xml.includes(tag)));
