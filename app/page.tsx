...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero"><div><p className="eyebrow">FOR AMAZON SELLERS · BROWSER-ONLY TOOL</p><h1>Add or verify the <em>synthetic performer</em> XMP tag.</h1><p className="lead">检查并添加 <code>{TAG}</code>，用于含有逼真、完全由 AI 生成的人物的商品媒体披露。文件只在您的浏览器中处理。</p><div className="hero-notes"><span>✓ JPEG & PNG</span><span>✓ No upload</span><span>✓ No duplicate tags</span></div></div><aside className="scope"><strong>适用范围</strong><p>面向 Amazon 商品图片、视频或 A+ 内容中含有逼真、完全由 AI 生成的人物的情形。</p><a href="#guide">先了解规则范围 ↓</a></aside></section>
    <section id="tool" className="tool-card"><div className="tool-intro"><p className="eyebrow">ONE FILE AT A TIME</p><h2>检测、添加、下载</h2><p>本工具不会判断人物是否属于规则范围；它只检查或写入元数据。请先根据 Amazon 规则完成内容判断。</p></div><div className="workbench"><input ref={input} onChange={selectFile} accept="image/jpeg,image/png,.jpg,.jpeg,.png" type="file" id="file" hidden /><button className="dropzone" onClick={() => input.current?.click()}><span className="plus">+</span><strong>{file ? file.name : "选择图片"}</strong><small>JPEG 或 PNG · 仅在此设备处理</small></button><div className={`result ${result.kind}`} aria-live="polite"><strong>{result.title}</strong><p>{result.detail}</p></div><button onClick={writeAndDownload} className="primary" disabled={!file || kind === "unsupported" || location?.compressed}>添加标签并下载 <span>→</span></button></div></section>
