...
export default function Home() {
  ...
  return <main>
    ...
    <section id="tool" className="workflow" aria-labelledby="workflow-heading"><div className="workflow-intro"><p className="eyebrow">{t("ONE AMAZON-SPECIFIC WORKFLOW", "单一 AMAZON 合规流程")}</p><h2 id="workflow-heading">{t("Write one exact tag. Verify every result.", "写入一个精确标签，逐个验证结果。")}</h2><p>{t("This is not a general metadata editor. It is purpose-built for the Amazon synthetic-performer disclosure workflow.", "这不是通用元数据编辑器；它只服务于 Amazon synthetic-performer 披露流程。")}</p></div>
      ...
      <section id="write-download" className="workspace-panel"><div className="panel-head"><b>03</b><h3>{t("Write and download", "写入并下载")}</h3><span className="count">{ready ? t(`${ready} ready to tag`, `${ready} 个待写入`) : t(`${completed} verified`, `已验证 ${completed} 个`)}</span></div><div className="action-row"><button className="primary" onClick={processAll} disabled={!canProcess || !tags.length}>{t("Write tags and verify", "写入并验证")}</button><button className="secondary" onClick={downloadZip} disabled={!completed}>{t("Download ZIP", "下载 ZIP")}</button><button className="text-button" onClick={() => setItems([])} disabled={!items.length}>{t("Clear", "清空")}</button></div><div className="file-list" aria-live="polite">{workflowItems.length ? workflowItems.map(item => <div className="file-row" key={item.id}><div><strong>{item.file.name}</strong><small>{fileDetail(item)}</small></div><span className={`file-status ${item.status}`}>{statusLabel(item.status)}</span>{item.output && <button className="download-one" onClick={() => download(item)}>{t("Download", "下载")}</button>}</div>) : <p className="empty-state">{items.length ? t("No selected files can be processed.", "没有可处理的已选文件。") : t("Add a batch of final JPG or PNG images to inspect their existing XMP before anything is written.", "添加一批最终 JPG 或 PNG 图片；写入前会先检查现有 XMP。")}</p>}</div>{unavailableItems.length > 0 && <details className="unavailable-files"><summary>{t(`${unavailableItems.length} file${unavailableItems.length === 1 ? "" : "s"} not available for processing`, `${unavailableItems.length} 个文件无法处理`)}</summary><div>{unavailableItems.map(item => <div className="unavailable-row" key={item.id}><div><strong>{item.file.name}</strong><small>{fileDetail(item)}</small></div><span className={`file-status ${item.status}`}>{statusLabel(item.status)}</span><button className="dismiss-file" onClick={() => removeItem(item.id)}>{t("Remove", "移除")}</button></div>)}</div></details>}</section>
    </section>
